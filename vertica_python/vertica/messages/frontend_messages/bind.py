...
class Bind(FrontendMessage):
    ...
    def to_bytes(self):
        bytes = pack('!{}sx{}sxHH'.format(len(self.portal_name), len(self.prepared_statement_name)), self.portal_name, self.prepared_statement_name, 0, len(self.parameter_values))
        for val in self.parameter_values.values():
            if val is None:
                bytes += pack('!I', [-1])
            else:
                bytes += pack('!I{}s'.format(len(val)), len(val), val)
        bytes += pack('!H', [0])
