...
class ErrorsTestCase(VerticaPythonUnitTestCase):
    def test_pickling(self):
        ...
        sql = "select 1;"
        exc = errors.VerticaSyntaxError(err_response, sql)

        serde = pickle.loads(pickle.dumps(exc))

        assert isinstance(serde, errors.VerticaSyntaxError)
        assert str(serde) == str(exc)
        assert serde.error_response == exc.error_response
